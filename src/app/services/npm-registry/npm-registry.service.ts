...
export class NpmRegistryService {
	...
	getRegistry(packageName: string, start: string, end: string): Observable<RegistryData> {
		...
		return forkJoin([this.getDownloadsPoint(packageName, start, end), this.getDownloadsRange(packageName, start, end)]).pipe(
			map(([total, range]) => ({ total, range, packageName, isHidden: false })),
	...
	protected getDownloadsPoint(packageName: string, start: string, end: string): Observable<number> {
		return this.httpClient
			.get<DownloadsPoint>(`${this.base}/downloads/point/${start}:${end}/${packageName}`)
			.pipe(map((res) => res.downloads));
	}

	protected getDownloadsRange(packageName: string, start: string, end: string): Observable<number[]> {
		return this.httpClient
			.get<DownloadsRange>(`${this.base}/downloads/range/${start}:${end}/${packageName}`)
